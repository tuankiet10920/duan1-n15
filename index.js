...
function goToAccountPage() {
  window.location.href =
    "http://localhost:8080/duan1-n15/index.php?page=account";
}
    ...
    let interval = setInterval(() => {
      ...
      if (totalSeconds <= 0) {
        ...
        countdownElement.innerHTML =
          "Mã otp của bạn đã hết hiệu lực, vui lòng quay lại!";
      }
    }, 1000);
  }
...
const products = [
  {
    id: 1,
    name: "Màn Hình Dell VG27AQ3A Full HD (1080p) 144 inch",
    image: "./imgduan1/dell1.jpg",
  },
  {
    id: 2,
    name: "Màn Hình Gaming ASUS TUF Gaming VG27AQ3A Full HD (1080p) 144 inch",
    image: "./imgduan1/dell1.jpg",
  },
];
...
const debouncedSearch = debounce(() => {
  ...
  if (query) {
    // call to api
    fetch(
      `http://localhost:8080/duan1-n15/index.php?page=getMonitor&findKey=${query}`
    )
      .then((response) => response.json())
      .then((json) => {
        if (json.length > 0) {
          searchResults.innerHTML = json
            .map(
              (product) => `
            <div class="search-results-item" onclick="goToProduct(${product.id_monitor})">
              <img src="public/img/${product.path_image}" alt="${product.name_image}" />
              <span>${product.name_monitor}</span>
            </div>
          `
            )
            .join("");
...
function goToProduct(id) {
  window.location.href = `http://localhost:8080/duan1-n15/index.php?page=detail&id=${id}`;
}
